        ...
        <div class="col-md-9">
            <div class="card text-center">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <h5 class="card-title"><b>{{ $value->name }} {{ $value->lastname }}</b></h5>
                    <p class="card-text"><b>{{ $value->position }}</b> en <b>{{ $value->company }}</b></p>                    
                    <form action="{{ route('clients.destroy', $value) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <a href="{{ route('clients.edit', $value) }}" class="btn btn-primary">Editar</a>
                ...
                <div class="card-footer text-muted">
                    <b>Teléfono:</b>{{ $value->phone }}    <b>Email:</b> {{ $value->email }}
                </div>
                <!-- deleteModal -->
