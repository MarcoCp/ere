        ...
        <div class="col-md-9">
            <div class="card text-center">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <h5 class="card-title"><b>{{ $value->project }}</b></h5>
                    <p class="card-text">Cliente: {{ $value->client->name }} {{ $value->client->lastname }}</p>                    
                    <form action="{{ route('quotes.destroy', $value) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <a href="{{ route('quotes.edit', $value) }}" class="btn btn-primary">Editar</a>
                ...
                <div class="card-footer text-muted">
                    <b>Fecha:</b>{{ $value->date }} <b>Presupuesto:</b>{{ $value->budget }}
                </div>
                <!-- deleteModal -->
